...
import CharacterNicknameSettings from "@/app/_components/CharacterNicknameSettings";
import CharacterManager from "@/app/_components/CharacterManager";
import { authOptions } from "@/app/lib/auth";
import { webPool } from "@/app/lib/db";
import { getUserProfileByEmail, upsertUser } from "@/app/lib/users";
...
export default async function CharactersPage() {
  ...
  return <main className="site-wash min-h-screen px-6 py-12 text-[#493647]"><section className="mx-auto max-w-5xl"><p className="text-sm font-semibold text-[#d45d91]">Voice With AI</p><h1 className="mt-3 text-4xl font-extrabold">{t.title}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#806579]">{t.description}</p><section className="mt-8"><CharacterManager initialCharacterId={result.rows[0]?.selected_character_id ?? "seline"} locale={locale} /></section><CharacterNicknameSettings initialNickname={profile?.nickname ?? ""} locale={locale} /><AssistantSettings locale={locale} title={t.settings} /></section></main>;
}
