...
export default async function CharactersPage() {
  ...
  return <main className="site-wash min-h-screen px-6 py-12 text-[#493647]"><section className="mx-auto max-w-5xl"><p className="text-sm font-semibold text-[#d45d91]">Discord Anime AI</p><h1 className="mt-3 text-4xl font-extrabold">{ko ? "캐릭터 관리" : "Character management"}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#806579]">{ko ? "대화할 AI 캐릭터를 선택하세요. 새 캐릭터는 준비되는 대로 이 목록에서 바로 선택할 수 있습니다." : "Choose the AI character you want to talk with. New characters will appear here when they are ready."}</p><section className="mt-8"><CharacterManager initialCharacterId={result.rows[0]?.selected_character_id ?? "seline"} locale={ko ? "ko" : "en"} /></section></section></main>;
}
