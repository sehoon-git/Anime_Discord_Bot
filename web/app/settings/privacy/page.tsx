...
export default async function PrivacySettingsPage() {
  ...
  return <main className="site-wash min-h-screen px-6 py-12 text-[#493647]"><section className="mx-auto max-w-3xl">
    ...
    <section className="mt-6 rounded-3xl border border-[#f0d7e5] bg-white/80 p-5 shadow-[0_16px_45px_rgba(198,135,169,0.1)]"><label className="flex items-center gap-3 text-base font-extrabold text-[#684b60]"><input type="checkbox" checked disabled readOnly className="h-4 w-4" /><span>{ko ? "필수 약관 동의" : "Required consents"}</span></label><div className="mt-5 space-y-4 text-sm">{[[ko ? "[필수] 음성 데이터 처리" : "[Required] Voice data processing", "/voice-policy"], [ko ? "[필수] 장기기억 저장" : "[Required] Long-term memory storage", "/privacy"], [ko ? "[필수] 개인정보 국외 이전" : "[Required] Overseas transfer of personal data", "/privacy"]].map(([text, href]) => <div key={href + text} className="flex items-center justify-between gap-3"><span className="flex items-center gap-3 font-semibold text-[#76566b]"><input type="checkbox" checked disabled readOnly />{text}</span><Link href={href} target="_blank" rel="noopener noreferrer" className="shrink-0 text-sm font-semibold text-[#d45d91] hover:text-[#b94c7d]">[{ko ? "상세보기" : "View details"}]</Link></div>)}</div></section>
  </section></main>;
}
