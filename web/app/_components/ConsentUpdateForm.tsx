...
export default function ConsentUpdateForm({ locale }: { locale: Locale }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error ?? (ko ? "동의 정보를 저장하지 못했습니다." : "We could not save your consent."));
      router.replace("/dashboard");
      router.refresh();
    } catch (caught) { setError(caught instanceof Error ? caught.message : (ko ? "동의 정보를 저장하지 못했습니다." : "We could not save your consent.")); }
    finally { setSaving(false); }
  }

  return <section className="mt-8 rounded-3xl border border-[#f0d7e5] bg-white/80 p-5 shadow-[0_16px_45px_rgba(198,135,169,0.12)]"><label className="flex items-center gap-3 text-base font-extrabold text-[#684b60]"><input type="checkbox" checked={allChecked} onChange={(event) => setAll(event.target.checked)} className="h-4 w-4 accent-[#e878ab]" /><span>{ko ? "약관 전체 동의" : "Agree to all terms"}</span></label><div className="mt-5 space-y-4 text-sm">{items.map(([key, text, href]) => <label key={key} className="flex items-center justify-between gap-3"><span className="flex items-center gap-3 font-semibold text-[#76566b]"><input type="checkbox" checked={consents[key]} onChange={(event) => setConsents((current) => ({ ...current, [key]: event.target.checked }))} className="accent-[#e878ab]" />{text}</span><Link href={href} target="_blank" rel="noopener noreferrer" className="shrink-0 text-sm font-semibold text-[#d45d91] hover:text-[#b94c7d]">[{ko ? "상세보기" : "View details"}]</Link></label>)}</div><button type="button" onClick={submit} disabled={!allChecked || saving} className="mt-6 w-full rounded-full border border-[#ef9fc2] bg-[#8f5f86] px-4 py-3 font-bold text-white shadow-[0_8px_20px_rgba(12,8,18,0.28)] transition-all hover:-translate-y-0.5 hover:bg-[#d879aa] active:translate-y-0 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60">{saving ? (ko ? "저장 중..." : "Saving...") : (ko ? "동의하고 계속하기" : "Agree and continue")}</button>{error ? <p className="mt-3 text-sm font-semibold text-red-500">{error}</p> : null}</section>;
}
