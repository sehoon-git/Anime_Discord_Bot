...
export default function AssistantSettings({ locale = "en-US" }: { locale?: Locale }) {
  ...
  </div><div className="mt-5 space-y-3 text-sm font-semibold">{toggles.map(([key, text]) => <label key={key} className="flex items-center justify-between"><span>{text}</span><input type="checkbox" checked={value[key]} onChange={(e) => set(key, e.target.checked)} /></label>)}</div><button type="button" onClick={save} disabled={saving} className="mt-5 w-full rounded-xl bg-[#e98ab5] px-4 py-3 font-bold text-white disabled:opacity-60">{saving ? t.saving : t.save}</button>{status ? <p className="mt-3 text-sm font-semibold text-[#a4577e]">{status}</p> : null}</section>;
}
