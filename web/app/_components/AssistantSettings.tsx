...
export default function AssistantSettings({ locale = "en-US", title }: { locale?: Locale; title?: string }) {
  ...
  </div><div className="mt-5 space-y-3 text-sm font-semibold">{toggles.map(([key, text]) => <div key={key} className="rounded-xl px-1 py-1"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span>{text}</span><button type="button" onClick={() => setExpandedSetting((current) => current === key ? null : key)} aria-expanded={expandedSetting === key} className="shrink-0 rounded-full border border-[#e3bfd3] px-2 py-0.5 text-xs font-semibold text-[#a4577e] transition hover:bg-white hover:text-[#d45d91]">{expandedSetting === key ? t.closeDetails : t.details}</button></div><input type="checkbox" checked={value[key]} onChange={(e) => set(key, e.target.checked)} /></div>{expandedSetting === key ? <p className="mt-2 rounded-lg bg-[#fff5fa] px-3 py-2 text-xs font-medium leading-5 text-[#806579]">{settingDescriptions[key]}</p> : null}</div>)}</div><button type="button" onClick={save} disabled={saving} aria-busy={saving} className="mt-5 w-full cursor-pointer rounded-full border border-[#ef9fc2] bg-[#8f5f86] px-4 py-3 font-bold text-white shadow-[0_8px_20px_rgba(12,8,18,0.28)] transition-all duration-150 hover:-translate-y-0.5 hover:bg-[#d879aa] hover:shadow-lg hover:shadow-pink-300/30 active:translate-y-0 active:scale-[0.98] disabled:cursor-wait disabled:transform-none disabled:opacity-60 disabled:shadow-none">{saving ? t.saving : t.save}</button>{status ? <p className="mt-3 text-sm font-semibold text-[#a4577e]">{status}</p> : null}</section>;
}
