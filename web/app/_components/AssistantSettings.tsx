...
export default function AssistantSettings({ locale = "en-US" }: { locale?: Locale }) {
  ...
  return <section className="mt-6 rounded-3xl border border-[#f0d7e5] bg-white/80 p-5 shadow-[0_16px_45px_rgba(198,135,169,0.1)]"><h2 className="text-lg font-bold text-[#684b60]">{t.title}</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
  ...
  </div><div className="mt-5 space-y-3 text-sm font-semibold">{toggles.map(([key, text]) => <label key={key} className="flex items-center justify-between"><span>{text}</span><input type="checkbox" checked={value[key]} onChange={(e) => set(key, e.target.checked)} /></label>)}</div><button type="button" onClick={save} disabled={saving} aria-busy={saving} className="mt-5 w-full cursor-pointer rounded-xl bg-[#e98ab5] px-4 py-3 font-bold text-white transition-all duration-150 hover:-translate-y-0.5 hover:bg-[#d96f9f] hover:shadow-lg hover:shadow-pink-300/30 active:translate-y-0 active:scale-[0.98] disabled:cursor-wait disabled:transform-none disabled:opacity-60 disabled:shadow-none">{saving ? t.saving : t.save}</button>{status ? <p className="mt-3 text-sm font-semibold text-[#a4577e]">{status}</p> : null}</section>;
}
