...
export default function AssistantSettings({ locale = "en-US", title }: { locale?: Locale; title?: string }) {
  ...
  const settingDescriptions: Record<string, string> = isKorean ? {
    memory_enabled: "대화 중 다음에도 도움이 될 만한 호칭·취향 같은 정보를 저장해, 이후 대화에 반영합니다.",
    sns_tone_enabled: "짧고 자연스러운 SNS 메시지처럼 조금 더 편한 말투로 답변합니다.",
    voice_response_enabled: "음성 채널에서 셀린의 답변을 음성으로 들려줍니다. 끄면 음성 답변을 하지 않습니다.",
    voice_summary_enabled: "대화의 핵심을 짧게 음성으로 요약해 들려주는 기능입니다. 현재는 준비 중인 기능입니다.",
    silent_notification_enabled: "대화가 조용한 시간에도 필요한 안내를 받을지 정합니다. 너무 자주 방해하지 않도록 빈도를 조절합니다.",
  } : {
  ...
  </div><div className="mt-5 grid gap-5 text-sm font-semibold md:grid-cols-[minmax(0,1fr)_minmax(18rem,0.9fr)]"><div className="space-y-2">{toggles.map(([key, text]) => <div key={key} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 rounded-xl px-1 py-1"><span className="truncate">{text}</span><button type="button" onClick={() => setExpandedSetting(key)} aria-pressed={expandedSetting === key} className={`rounded-full border px-2 py-0.5 text-xs font-semibold transition ${expandedSetting === key ? "border-[#d45d91] bg-[#fff0f7] text-[#d45d91]" : "border-[#e3bfd3] text-[#a4577e] hover:bg-white hover:text-[#d45d91]"}`}>{t.details}</button><input type="checkbox" checked={value[key]} onChange={(e) => set(key, e.target.checked)} /></div>)}</div><aside className="min-h-28 rounded-2xl border border-[#efd8e5] bg-[#fff5fa] p-4"><p className="text-xs font-bold text-[#a4577e]">{isKorean ? "설정 설명" : "Setting details"}</p>{expandedSetting ? <><h3 className="mt-2 text-sm font-bold text-[#684b60]">{toggles.find(([key]) => key === expandedSetting)?.[1]}</h3><p className="mt-2 text-sm font-medium leading-6 text-[#806579]">{settingDescriptions[expandedSetting]}</p></> : <p className="mt-2 text-sm font-medium leading-6 text-[#806579]">{isKorean ? "궁금한 설정의 ‘상세 보기’를 누르면 여기에 설명이 표시됩니다." : "Select Details next to a setting to see an explanation here."}</p>}</aside></div><button type="button" onClick={save} disabled={saving} aria-busy={saving} className="mt-5 w-full cursor-pointer rounded-full border border-[#ef9fc2] bg-[#8f5f86] px-4 py-3 font-bold text-white shadow-[0_8px_20px_rgba(12,8,18,0.28)] transition-all duration-150 hover:-translate-y-0.5 hover:bg-[#d879aa] hover:shadow-lg hover:shadow-pink-300/30 active:translate-y-0 active:scale-[0.98] disabled:cursor-wait disabled:transform-none disabled:opacity-60 disabled:shadow-none">{saving ? t.saving : t.save}</button>{status ? <p className="mt-3 text-sm font-semibold text-[#a4577e]">{status}</p> : null}</section>;
}
