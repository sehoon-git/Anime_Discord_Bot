    ...
    <div className="mt-4 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
      {plans.map((plan, index) => {
        ...
        return <article key={plan.code} className={`plan-card group ${isCurrent ? "plan-card-current" : plan.code === "more-like" ? "plan-card-pro" : "plan-card-love"}`}>
          ...
          <ul className="mt-7 space-y-4 text-sm text-[#76566b]">{planCopy.features.map((feature) => <li key={feature} className={`flex items-center gap-3 ${plan.code === "love" && feature === "우선 제공되는 풍부한 음성 대화" ? "-mr-8 whitespace-nowrap" : ""}`}><span className="check-mark">✓</span>{feature}</li>)}</ul>
