...
export default function HeaderMenu({ locale }: { locale: Locale }) {
  ...
  function authenticate() {
    ...
    else signIn("google", { callbackUrl: "/profile" });
  }
    ...
    <div ref={menuRef} className="relative">
      ...
      {isOpen ? (
        <div className="theme-menu absolute right-0 top-12 z-50 w-44 overflow-hidden rounded-2xl border border-[#efd4e2] bg-white py-2 shadow-xl shadow-pink-200/30">
          ...
          <button type="button" disabled={status === "loading"} onClick={authenticate} className="theme-menu-item block w-full px-4 py-3 text-left text-sm font-semibold text-[#76566b] hover:bg-[#fff0f7] disabled:cursor-wait">
            {status === "loading" ? (ko ? "확인 중..." : "Checking...") : session?.user ? (ko ? "로그아웃" : "Log out") : (ko ? "로그인" : "Log in")}
          </button>
        </div>
      ) : null}
    </div>
