...
import MemoryEditButton from "@/app/_components/MemoryEditButton";
import MemoryManualAddForm from "@/app/_components/MemoryManualAddForm";
  ...
  return <main className="site-wash min-h-screen px-6 py-16 text-[#493647]">
    <section className="mx-auto max-w-4xl">
      ...
      <section className="mt-10 rounded-3xl border border-[#f0d7e5] bg-white/80 p-6 shadow-[0_16px_45px_rgba(198,135,169,0.12)]">
        <MemoryManualAddForm locale={locale} />
        {memories.length === 0 ? <div className="mt-6 grid items-center gap-6 md:grid-cols-[1fr_auto]"><div><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#ef8fba] to-[#a895f4] text-2xl text-white shadow-lg">✦</span><h2 className="mt-5 text-xl font-extrabold">{copy.emptyTitle}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[#92768a]">{copy.emptyText}</p></div><Link href="/characters" className="inline-flex rounded-full bg-gradient-to-r from-[#ef8fba] to-[#a895f4] px-5 py-3 text-sm font-extrabold text-white shadow-lg transition hover:-translate-y-0.5">{copy.character} →</Link></div> : <><div className="mt-6 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-extrabold">{copy.savedTitle}</h2><p className="mt-1 text-sm text-[#92768a]">{copy.savedText}</p></div><span className="rounded-full bg-[#fff0f7] px-3 py-1.5 text-sm font-bold text-[#d45d91]">{copy.keeping(memories.length)} / {copy.items(memoryLimit)}</span></div><div className="mt-6 space-y-3">{memories.map((memory) => <article key={memory.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-[#efd8e5] bg-[#fffafd]/70 p-5 sm:flex-row sm:items-center"><div><p className="font-semibold">{memory.isPinned ? "📌 " : ""}{memory.content}</p><p className="mt-2 text-sm text-[#92768a]">{memory.source === "manual" ? (locale === "ko-KR" ? "직접 입력됨" : locale === "ja-JP" ? "直接入力" : "Added by you") : `${copy.confidence} ${Math.round(memory.confidence * 100)}%`} · {new Date(memory.createdAt).toLocaleString(locale)}</p></div><div className="flex shrink-0 flex-wrap gap-2"><MemoryEditButton memoryId={memory.id} content={memory.content} locale={locale} /><form action={toggleMemoryPin}><input type="hidden" name="memoryId" value={memory.id} /><input type="hidden" name="pinned" value={String(!memory.isPinned)} /><button type="submit" className="rounded-full border border-pink-300 px-4 py-2 text-sm font-semibold text-pink-700 transition hover:bg-pink-100">{memory.isPinned ? copy.unpin : copy.pin}</button></form><form action={deleteOneMemory}><input type="hidden" name="memoryId" value={memory.id} /><button type="submit" className="rounded-full border border-red-300 px-5 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50">{copy.remove}</button></form></div></article>)}</div><form action={deleteEveryMemory} className="mt-6 border-t border-[#f0d7e5] pt-5"><button type="submit" className="rounded-full bg-red-500 px-6 py-3 text-sm font-bold text-white transition hover:bg-red-50">{copy.clear}</button></form></>}
      </section>
