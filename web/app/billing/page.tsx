...
function BillingDashboard({ billing, locale }: { billing: BillingStatus; locale: Locale }) {
  const en = locale === "en";
  return <main className="site-wash min-h-screen px-6 py-3 text-[#493647]"><section className="mx-auto max-w-6xl"><div className="billing-intro billing-intro-compact text-center"><span className="billing-sparkle billing-sparkle-compact">✦</span><p className="text-sm font-bold text-[#d45d91]">Discord Anime AI · {en ? "plans" : "plans"}</p><h1 className="mt-1 text-4xl font-extrabold tracking-[-0.03em] text-[#5b4054]">{en ? "A plan that fits your conversations" : "나에게 맞는 대화 플랜"}</h1><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#92768a]">{en ? "Choose the plan that fits how often you talk." : "대화 빈도와 이용 방식에 맞는 플랜을 골라보세요."}</p></div><section className="mt-2"><BillingPlans currentPlanCode={billing.plan.code} locale={locale} showCreditPanel={false} /></section></section></main>;
}
