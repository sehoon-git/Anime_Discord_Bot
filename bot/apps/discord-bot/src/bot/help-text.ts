...
export function localConsoleHelp(): string[] {
  return [
    'announce <message> - Send a system announcement to every server.',
    '로컬 콘솔 명령어',
    'help - 이 콘솔에서 사용할 수 있는 명령어와 설명을 표시합니다.',
    'status - Discord 연결 상태를 확인합니다.',
    'status <서버 ID> - Discord 연결 상태와 해당 서버의 음성 세션 상태를 확인합니다.',
    'logs compact - 오류와 핵심 처리 시간만 간단히 표시합니다. (기본값)',
    'logs detail - 음성/대화/TTS 파이프라인과 API 요청·응답을 상세히 표시합니다. API 키는 표시하지 않습니다.',
    'voice leave <서버 ID> - 해당 서버에서 진행 중인 Seline 음성 세션을 종료합니다.',
    'clear - 콘솔에 표시된 로그를 지웁니다.',
    'restart - 같은 콘솔 창에서 Seline을 안전하게 재시작합니다.',
